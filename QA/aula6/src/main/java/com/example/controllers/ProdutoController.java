...
public class ProdutoController {
    ...
    private void salvarProduto() {
        ...
        try (Connection conn = Database.getConnection();
             PreparedStatement stmt = conn.prepareStatement("INSERT INTO Produto (produto_nome, produto_quantidade, produto_preco, produto_localizacao, produto_categoria) VALUES (?, ?, ?, ?, ?)")) {
    ...
    private void atualizarProduto() {
        ...
        try (Connection conn = Database.getConnection();
             PreparedStatement stmt = conn.prepareStatement("UPDATE Produto SET nome = ?, quantidade = ?, preco = ?, localizacao = ?, categoria = ? WHERE id = ?")) {
    ...
    private void carregarProdutos() {
        ...
        try (Connection conn = Database.getConnection();
            ...
            while (rs.next()) {
                listaProdutos.add(new Produto(rs.getInt("produto_id"), rs.getString("produto_nome"), rs.getInt("produto_quantidade"), rs.getDouble("produto_preco"), rs.getString("produto_localizacao"), rs.getString("produto_categoria"), null, 0));
            }
            tableProdutos.setItems(listaProdutos);
        } catch (SQLException e) {
            e.printStackTrace();
